...
public class ChatActionConverterTests
{
    ...
    [Fact]
    public void Should_Convert_String_ToChatActionFallbackUnsupported_For_Incorrect_ChatAction()
    {
        string jsonData = """{"chat_id":1234,"action":"very_very_new"}""";


        SendChatActionRequest? result = JsonSerializer.Deserialize(jsonData, TelegramBotClientJsonSerializerContext.Instance.SendChatActionRequest);

        Assert.NotNull(result);
        Assert.Equal(ChatAction.FallbackUnsupported, result.Action);
    }
    ...
    public void Should_Throw_JsonException_For_Incorrect_ChatAction()
    {
        Assert.Throws<JsonException>(() =>
                JsonSerializer.Serialize(ChatActionData.NewSendChatActionRequest((ChatAction)int.MaxValue),
                    TelegramBotClientJsonSerializerContext.Instance.SendChatActionRequest));
    }

    private class ChatActionData : IEnumerable<object[]>
    {
        internal static SendChatActionRequest NewSendChatActionRequest(ChatAction chatAction)
        {
