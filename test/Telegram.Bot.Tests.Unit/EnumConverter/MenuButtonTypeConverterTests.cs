...
public class MenuButtonTypeConverterTests
{
    ...
    {
        MenuButton? result = JsonSerializer.Deserialize( """{"type":"very_very_new"}""", TelegramBotClientJsonSerializerContext.Instance.MenuButton);

        Assert.NotNull(result);
        Assert.Equal(MenuButtonType.FallbackUnsupported, result.Type);
        Assert.IsType<MenuButtonFallbackUnsupported>(result);
    }
