...
public class ChatBoostSourceTypeConverterTests
{
    ...
    {
        string jsonData ="""{"source":"very_very_new","giveaway_message_id":1}""";
        ChatBoostSource? result = JsonSerializer.Deserialize(jsonData, TelegramBotClientJsonSerializerContext.Instance.ChatBoostSource);

        Assert.NotNull(result);
        Assert.IsType<ChatBoostSourceFallbackUnsupported>(result);
        Assert.Equal(ChatBoostSourceType.FallbackUnsupported, result.Source);
    }
