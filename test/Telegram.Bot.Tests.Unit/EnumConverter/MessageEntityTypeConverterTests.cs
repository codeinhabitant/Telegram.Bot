...
public class MessageEntityTypeConverterTests
{
    ...
    public void Should_Verify_All_MessageEntityType_Members()
    {
        List<string> messageEntityTypeMembers = Enum
            ...
            .Except(new [] { MessageEntityType.FallbackUnsupported.ToString() })
            .OrderBy(x => x)
            .ToList();
    ...
    {
        string json  = $$"""{"type":"very_very_new","offset":0,"length":10}""";
        MessageEntity? result = JsonSerializer.Deserialize(json, TelegramBotClientJsonSerializerContext.Instance.MessageEntity)!;

        Assert.NotNull(result);
        Assert.Equal(MessageEntityType.FallbackUnsupported, result.Type);
    }
