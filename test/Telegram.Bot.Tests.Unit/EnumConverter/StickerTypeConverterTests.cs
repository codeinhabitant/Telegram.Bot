...
public class StickerTypeConverterTests
{
    ...
    public void Should_Verify_All_StickerType_Members()
    {
        List<string> stickerTypeMembers = Enum
            ...
            .Except(new [] { StickerType.FallbackUnsupported.ToString() })
            .OrderBy(x => x)
            .ToList();
    ...
    {
        string jsonData =
            """{"type":"very_very_new","width":512,"height":512,"is_animated":false,"is_video":false,"file_id":"1","file_unique_id":"1"}""";

        Sticker? result = JsonSerializer.Deserialize(jsonData, TelegramBotClientJsonSerializerContext.Instance.Sticker)!;

        Assert.NotNull(result);
        Assert.Equal(StickerType.FallbackUnsupported, result.Type);
    }
