...
public class ChatMemberStatusConverterTests
{
    ...
    {
        string json = """{"status":"very_very_new","user":{"id":1,"is_bot":false,"first_name":"FirstName"}}""";

        ChatMember? result =
            JsonSerializer.Deserialize(json, TelegramBotClientJsonSerializerContext.Instance.ChatMember);

        Assert.NotNull(result);
        Assert.Equal(ChatMemberStatus.FallbackUnsupported, result.Status);
        Assert.IsType<ChatMemberFallbackUnsupported>(result);
    }
