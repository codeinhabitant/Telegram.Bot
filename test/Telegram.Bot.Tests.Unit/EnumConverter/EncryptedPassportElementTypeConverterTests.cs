...
public class EncryptedPassportElementTypeConverterTests
{
    ...
    {
        string json = """{"type":"very_very_new","hash":"hash"}""";
        EncryptedPassportElement? result =
            JsonSerializer.Deserialize(json, TelegramBotClientJsonSerializerContext.Instance.EncryptedPassportElement);

        Assert.NotNull(result);
        Assert.Equal(EncryptedPassportElementType.FallbackUnsupported, result.Type);
    }
