...
public class EmojiConverterTests
{
    ...
    [Fact]
    public void Should_Return_FallbackUnsupported_For_Incorrect_Emoji()
    {
        string jsonData = $$"""{"chat_id":1,"emoji":"very_very_new"}""";

        SendDiceRequest? result = JsonSerializer.Deserialize(jsonData, TelegramBotClientJsonSerializerContext.Instance.SendDiceRequest);

        Assert.NotNull(result);
        Assert.Equal(Emoji.FallbackUnsupported, result.Emoji);
    }
    ...
    private class EmojiData : IEnumerable<object[]>
    {
