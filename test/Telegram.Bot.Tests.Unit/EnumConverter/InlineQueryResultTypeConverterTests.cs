...
public class InlineQueryResultTypeConverterTests
{
    ...
    {
        string json =  """{"type":"very_very_new","game_short_name":"game","id":"id"}""";
        InlineQueryResult? result = JsonSerializer.Deserialize(json, TelegramBotClientJsonSerializerContext.Instance.InlineQueryResult);

        Assert.NotNull(result);
        Assert.Equal(InlineQueryResultType.FallbackUnsupported, result.Type);
        Assert.IsType<InlineQueryResultFallbackUnsupported>(result);
    }
